...
{{--                                <p class="m-b-0">選考結果：{{$jobProgressStatus[$applicant->progress_status]}}。{{date('Y/m/d', strtotime($applicant->seq_update_dt."+".((int)$jobFlow['attention_condition'])." day"))}}を目処に選考結果をお待ちください。</p>--}}
                                <p class="m-b-0">選考結果：{{$jobProgressStatus[$jobFlow['progress_status']]}}。{{\Carbon\Carbon::now()->addDay((int)$jobFlow['attention_condition'])->format('Y/m/d')}}を目処に選考結果をお待ちください。</p>
                                {{--{{dd($jobFlow)}}--}}
                            </div>
