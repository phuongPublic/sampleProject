...
@endif
    <div class="float-left text-left w-22 name">
        <img width="50" class="m-r-10" src="{{route('supply.user.avatar', ['id' => $item->demand_user_id])}}">
    ...
    <div class="float-left text-center w-10 progress_status border-left">
        ...
        @if($jobFlow[$item->seq] && $jobFlow[$item->seq]['help_message'])
            <img data-toggle="tooltip" data-html="true" data-placement="bottom" title= '{{$jobFlow[$item->seq]['help_message']}}' class="show-tooltip" width="15" src="/img/help.png">
        @endif
    </div>
    ...
    <div class="float-left w-40 seq border-right border-left">
    ...
    <div class="float-left text-right no-center w-13 action p-t-15 ">
    ...
    <div class="float-left text-right no-center p-t-15 w-5 action">
